# Script para migrar el nuevo frontend a la raiz del proyecto
...
Write-Host "  [OK] $movedCount archivos/carpetas movidos" -ForegroundColor Green

# Paso 3: Mover contenido de frontend/ a la raiz
Write-Host ""
Write-Host "[3/7] Moviendo nuevo frontend a la raiz..." -ForegroundColor Yellow
...
foreach ($item in $newFrontendItems) {
    ...
    # Si el archivo ya existe en la raiz, eliminarlo primero
...
Write-Host "  [OK] $movedNewCount archivos/carpetas movidos a la raiz" -ForegroundColor Green

# Paso 4: Eliminar carpeta frontend vacia
Write-Host ""
Write-Host "[4/7] Limpiando carpeta frontend..." -ForegroundColor Yellow
if (Test-Path "frontend") {
    # Verificar si esta vacia o solo tiene node_modules/.next
    ...
    if ($remainingItems.Count -eq 0) {
        ...
        Write-Host "  [OK] Carpeta frontend eliminada" -ForegroundColor Green
    } else {
        Write-Host "  [WARN] Carpeta frontend no esta vacia, no se elimino" -ForegroundColor Yellow
...
# Paso 6: Crear/actualizar archivo de variables de entorno para produccion
Write-Host ""
...
Write-Host "  [OK] .env.production creado" -ForegroundColor Green
...
foreach ($item in $requiredItems) {
    if (Test-Path $item) {
        ...
        Write-Host "  [ERROR] $item NO ENCONTRADO" -ForegroundColor Red
        $allGood = $false
    }
}
...
Write-Host "  [OK] Nuevo frontend en raiz (src/, public/, etc.)" -ForegroundColor Green
Write-Host "  [OK] Frontend antiguo respaldado en old-frontend/" -ForegroundColor Green
Write-Host ""
...
Write-Host "  - El frontend antiguo esta en old-frontend/" -ForegroundColor Gray
